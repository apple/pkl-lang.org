...
Thanks to https://github.com/StefMa[StefMa] for their work!

At the most basic level, defining a workflow in Pkl is almost the same as defining a workflow in YAML.
Except, the editor provides code-completion, documentation, and typechecking.
...
GitHub's core actions (actions that are within https://github.com/actions[actions org]).
...
Additionally, it uses a dummy workflow called `+__lockfile__+.yml` to help it generate your workflows.
...
So, when we adjust the logic in <<abstraction-layer,our abstraction layer>>, we use script helpers to generate pull requests, and for someone else to approve.
...
Try out writing your own actions using Pkl!
